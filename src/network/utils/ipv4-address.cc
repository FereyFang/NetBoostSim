...
Ipv4Address
Ipv4Address::GetIVIAddressFromIpv6 (Ipv6Address addr, Ipv6Prefix prefix)
{
  uint8_t buf[4];
  uint8_t ipv6_buf[12];
  addr.GetBytes (ipv6_buf);
  uint8_t prefix_len = prefix.GetPrefixLength();
  if (prefix_len % 8 == 0){
      for (int i = 0; i < 4 ; ++i)
        {
          buf[i] = ipv6_buf[prefix_len/8+i];
        }
    }
  else{
      uint8_t start_prefix = prefix_len/8;
      uint8_t start_bit = prefix_len%8;
      for (int i = 0; i < 4; ++i)
        {
          uint8_t start_byte = (ipv6_buf[start_prefix] << start_bit);
          start_prefix++;
          start_byte = start_byte | (ipv6_buf[start_prefix] >> (8 - start_bit));
        }
    }
