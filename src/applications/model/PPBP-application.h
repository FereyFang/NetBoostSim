...
namespace ns3 {
	...
	private:
		...
		Ptr<RandomVariableStream>	m_burstArrivals;	// Mean rate of burst arrivals (Possion lambda)
		Ptr<RandomVariableStream>   m_burstLength;		// Mean burst time length (Pareto mean)
		DataRate        m_cbrRate;						// Burst intensity (constant bit-rate)

		double			m_h;							// Hurst parameter	(Pareto distribution)
		double			m_shape;						// Shape			(Pareto distribution shape)
		Time			m_timeSlot;						// The time slot （Pareto scale）
