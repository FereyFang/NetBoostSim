...
{ 
  static TypeId tid = TypeId ("ns3::Ipv4GlobalRouting")
    ...
    .AddAttribute ("EcmpMode",
                   ...
                   EnumValue(ECMP_PER_FLOW),
                   MakeEnumAccessor(&Ipv4GlobalRouting::m_ecmpType),
...
Ipv4GlobalRouting::LookupGlobal (Ipv4Address dest, Ptr<NetDevice> oif)
{
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
//      // add for debug ecmp
//      Ptr<Node> node = m_ipv4->GetNetDevice (0)->GetNode();
//
//      if (node->GetId()==10){
//          NS_LOG_WARN ("Router 0 are looking for routes for "<<dest<<" with ECMP "<<m_randomEcmpRouting);
//        }
