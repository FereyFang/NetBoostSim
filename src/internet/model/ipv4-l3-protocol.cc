...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  /* IVI: translate the IP header from IPv4 to IPv6 */
  if (rtentry->IsTranslation()){

      Ipv6Header v6_header = TranslateHeader4To6 (ipHeader,rtentry);
      Ptr<Ipv6> node_ipv6 = (this->GetObject<Node> ())->GetObject<Ipv6> ();

      Ptr<NetDevice> ipv6_device = rtentry->GetOutputDevice();
      NS_LOG_LOGIC ("IPv4 IVI translation "<<ipv6_device->GetAddress());

      Ptr<Ipv6Route> v6_rtentry = Create<Ipv6Route> ();
      v6_rtentry->SetGateway (Ipv6Address::GetAny ());
      v6_rtentry->SetOutputDevice (ipv6_device);
      node_ipv6->SendRealOut(v6_rtentry,packet,v6_header);
    }else{
      SendRealOut (rtentry, packet, ipHeader);
    }
}
...
Ipv6Header
Ipv4L3Protocol::TranslateHeader4To6 (const Ipv4Header &header, Ptr<Ipv4Route> rtentry) const
{
  Ipv6Header new_header = Ipv6Header();
  new_header.SetTrafficClass (header.GetTos());
  new_header.SetPayloadLength (header.GetPayloadSize());
  new_header.SetHopLimit (header.GetTtl());
  new_header.SetNextHeader (header.GetProtocol());

  // translate address
  new_header.SetSourceAddress (Ipv6Address::MakeIVIIpv4MappedAddress (header.GetSource(),
                                                                      rtentry->GetSrcPrefixAddr(),
                                                                      rtentry->GetSrcPrefix()));
  new_header.SetDestinationAddress(Ipv6Address::MakeIVIIpv4MappedAddress (header.GetDestination(),
                                                                           rtentry->GetDstPrefixAddr(),
                                                                           rtentry->GetDstPrefix()));
  return new_header;
}
