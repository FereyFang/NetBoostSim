      ...
      if (mask.IsMatch (dest, entry)) 
        {
          ...
          NS_LOG_LOGIC("Find Route :"<<(*route));
          NS_LOG_LOGIC("IVI translation route:"<<(route->IsTranslation())<<(route->GetSrcIpv6Address()));
          /* IVI translation routing table entry */
          if (route->IsTranslation()){
              rtentry->SetSrcPrefixAddr (route->GetSrcIpv6Address());
              rtentry->SetDstPrefixAddr (route->GetDstIpv6Address());
              rtentry->SetSrcPrefix (route->GetSrcIpv6Prefix());
              rtentry->SetDstPrefix (route->GetDstIpv6Prefix());

              // DEBUG ruiyu: for translation entry, the interfaceIdx is for IPv6 associate interface
              Ptr<Ipv6> node_ipv6 = (m_ipv4->GetObject<Node> ())->GetObject<Ipv6> ();
              rtentry->SetOutputDevice (node_ipv6->GetNetDevice (interfaceIdx));
              NS_LOG_LOGIC(" IPv4 LookupStatic of Node "<< m_ipv4->GetObject<Node> ()->GetId()<<
                            " the ourput device for the IVI translation route "<< route->GetDest()<<
                            " : ifindex "<< interfaceIdx<<" device pointer "<< node_ipv6->GetNetDevice(interfaceIdx));
            }
          else
            {
...
void
Ipv4StaticRouting::AddNetworkRouteTo (Ipv4Address network, Ipv4Mask networkMask,
                                      Ipv4Address nextHop, uint32_t interface,
                                      Ipv6Address srcPrefixAddr, Ipv6Address dstPrefixAddr,
                                      Ipv6Prefix srcPrefix, Ipv6Prefix dstPrefix, uint32_t metric)
{
  NS_LOG_FUNCTION (this << network << " " << networkMask << " " << nextHop << " " << interface << " "
                        << srcPrefixAddr<<" "<<srcPrefix<<" "<<dstPrefixAddr<<" "<<dstPrefix<<" "<< metric);
...
void
Ipv4StaticRouting::AddNetworkRouteTo (Ipv4Address network, Ipv4Mask networkMask, uint32_t interface,
                                      Ipv6Address srcPrefixAddr, Ipv6Address dstPrefixAddr,
                                      Ipv6Prefix srcPrefix, Ipv6Prefix dstPrefix, uint32_t metric)
{
...
void
Ipv4StaticRouting::AddHostRouteTo (Ipv4Address dest, Ipv4Address nextHop, uint32_t interface,
                                   Ipv6Address srcPrefixAddr, Ipv6Address dstPrefixAddr,
                                   Ipv6Prefix srcPrefix, Ipv6Prefix dstPrefix, uint32_t metric)
{
  NS_LOG_FUNCTION (this << dest << " " << nextHop << " " << interface << " "
                        << srcPrefixAddr<<" "<<srcPrefix<<" "<<dstPrefixAddr<<" "<<dstPrefix<<" "
                        << metric);
  AddNetworkRouteTo (dest, Ipv4Mask::GetOnes (), nextHop, interface,
                     srcPrefixAddr,dstPrefixAddr,srcPrefix,dstPrefix,metric);
}
void
Ipv4StaticRouting::AddHostRouteTo (Ipv4Address dest, uint32_t interface, Ipv6Address srcPrefixAddr,
                                   Ipv6Address dstPrefixAddr, Ipv6Prefix srcPrefix,
                                   Ipv6Prefix dstPrefix, uint32_t metric)
{
  NS_LOG_FUNCTION (this << dest  << " " << interface << " "
                        << srcPrefixAddr<<" "<<srcPrefix<<" "<<dstPrefixAddr<<" "<<dstPrefix<<" "
                        << metric);
  AddNetworkRouteTo (dest, Ipv4Mask::GetOnes (), interface,
                     srcPrefixAddr,dstPrefixAddr,srcPrefix,dstPrefix,metric);
}
