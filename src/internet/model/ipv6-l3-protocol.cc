...
void Ipv6L3Protocol::IpForward (Ptr<const NetDevice> idev, Ptr<Ipv6Route> rtentry, Ptr<const Packet> p, const Ipv6Header& header)
{
  ...
  /* IVI: translate the IP header from IPv6 to IPv4 */
  NS_LOG_INFO("IPv6-l3-protocol IpForward: IVI translation for node :"<< m_node->GetId ()<<" from "<<ipHeader.GetSource()<<" to "<< ipHeader.GetDestination());
  if (rtentry->isTranslation()){
      NS_LOG_INFO ("IPv6-l3-protocol IpForward: need IVI "<<ipHeader.GetDestination());
      Ipv4Header v4_header = TranslateHeader6To4 (ipHeader,rtentry);

      Ptr<Ipv4> node_ipv4 = (this->GetObject<Node> ())->GetObject<Ipv4> ();
      Ptr<NetDevice> ipv4_device = rtentry->GetOutputDevice();

      Ptr<Ipv4Route> v4_route = Create<Ipv4Route>();
      v4_route->SetGateway (Ipv4Address::GetAny());
      v4_route->SetOutputDevice (ipv4_device);

      node_ipv4->SendRealOut (v4_route,packet,v4_header);
    }else{
...
Ipv4Header
Ipv6L3Protocol::TranslateHeader6To4 (Ipv6Header header, Ptr<Ipv6Route> rtentry) const
{
  Ipv4Header ipHeader;
  ipHeader.SetSource (Ipv4Address::GetIVIAddressFromIpv6 (header.GetSourceAddress(),rtentry->GetIVIPrefix()));
  ipHeader.SetDestination (Ipv4Address::GetIVIAddressFromIpv6 (header.GetDestinationAddress(),rtentry->GetIVIPrefix()));
  ipHeader.SetProtocol (header.GetNextHeader());
  ipHeader.SetPayloadSize (header.GetPayloadLength());
  ipHeader.SetTtl (header.GetHopLimit());
  ipHeader.SetTos (header.GetTrafficClass());
