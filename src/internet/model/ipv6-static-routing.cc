...
void Ipv6StaticRouting::AddNetworkRouteTo (Ipv6Address network, Ipv6Prefix networkPrefix, Ipv6Address nextHop, uint32_t interface, uint32_t metric,bool isTranslation)
{
  NS_LOG_FUNCTION (this << network << networkPrefix << nextHop << interface << metric<<isTranslation);
  Ipv6RoutingTableEntry route;
  if (isTranslation){
      route = Ipv6RoutingTableEntry::CreateNetworkRouteTo (network, networkPrefix, nextHop, interface,isTranslation);
    }else{
...
void Ipv6StaticRouting::AddIVINetworkRouteTo(Ipv6Address network,Ipv6Prefix networkPrefix, Ipv6Prefix iviPrefix,uint32_t interface,uint32_t metric, bool isTranslation)
{
  NS_LOG_FUNCTION (this << network << networkPrefix << iviPrefix << interface << metric<<isTranslation);
  Ipv6RoutingTableEntry route;
  route = Ipv6RoutingTableEntry::CreateIVINetworkRouteTo (network, networkPrefix, interface,isTranslation,iviPrefix);
...
void Ipv6StaticRouting::AddNetworkRouteTo (Ipv6Address network, Ipv6Prefix networkPrefix, Ipv6Address nextHop, uint32_t interface, Ipv6Address prefixToUse, uint32_t metric,bool isTranslation)
{
  NS_LOG_FUNCTION (this << network << networkPrefix << nextHop << interface << prefixToUse << metric<<isTranslation);
  ...
  Ipv6RoutingTableEntry route;
  if (isTranslation){
      route = Ipv6RoutingTableEntry::CreateNetworkRouteTo (network, networkPrefix, nextHop, interface, prefixToUse,isTranslation);
    }else{
...
void Ipv6StaticRouting::AddNetworkRouteTo (Ipv6Address network, Ipv6Prefix networkPrefix, uint32_t interface, uint32_t metric,bool isTranslation)
{
  NS_LOG_FUNCTION (this << network << networkPrefix << interface <<isTranslation);

  Ipv6RoutingTableEntry route;
  if (isTranslation){
      route = Ipv6RoutingTableEntry::CreateNetworkRouteTo (network, networkPrefix, interface,isTranslation);
    }else{
          ...
          if (!interface || interface == m_ipv6->GetNetDevice (j->GetInterface ()))
            {
              ...
              if (route->IsTranslation()){
                  rtentry->SetSource(m_ipv6->SourceAddressSelection(1,route->GetGateway()));
                }
              else{
              ...
              if (route->IsTranslation()){
                  rtentry->SetIVIPrefix (route->GetIviPrefix());
                  NS_LOG_LOGIC ("Ipv6StaticRouting Find IVI routing :"<<rtentry->GetDestination ()<<
                                " IVI Prefix :"<<rtentry->GetIVIPrefix());


                  // DEBUG ruiyu: for translation entry, the interfaceIdx is for IPv4 associate interface
                  Ptr<Ipv4> node_ipv4 = (m_ipv6->GetObject<Node>())->GetObject<Ipv4>();
                  rtentry->SetOutputDevice (node_ipv4->GetNetDevice (interfaceIdx));
                }
              else{
